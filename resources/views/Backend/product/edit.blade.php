...
@section('master_content')
    <div class="container">
        <div class="card">
            <h2 class="m-3 d-inline">Products Edit</h2>
            ...
            <div class="card-body ">
                <form action="{{ route('admin.product.update', $product->id) }}" method="POST">
                        ...
                        @if ($product->attribute)
                            @foreach ($product->attribute as $attribute)
                                <table class="table table-bordered" id="dynamic__degree">
                                    <tr>
                                        <td>
                                            <input type="text" class="form-control form-control-sm key_list"
                                                placeholder="Degree" id="key" name="attribute[{{ $ed_counter }}][key]"
                                                value="{{ $attribute['key'] }}">
                                        </td>
                                        <td>
                                            <input type="text" class="form-control form-control-sm value_list"
                                                placeholder="Institution" id="value"
                                                name="attribute[{{ $ed_counter }}][value]"
                                                value="{{ $attribute['value'] }}">
                                        </td>
                                        <td>
                                            <button type="button" id="degree__add" class="btn btn-success" disabled> <i
                                                    class="fa fa-plus-circle" disabled=""></i>
                                            </button>
                                        </td>
                                    </tr>
                                </table>
                    ...
                    <div class="my-2">
                        <button class="btn btn-success btn-sm">Update Product</button>
                    </div>
                </form>
            </div>
        </div>
